...
class TencentCloudTim
{
    ...
    public function sendPrivateMsg($to, TencentCloudImMessage $message)
    {
        try {
            ...
            if (is_array($to)) {
                $request = new BatchSendMsgRequest($to, random_int(1, 999999), $msgBody, $message->getFromAccount());
            } else {
                $request = new SendMsgRequest($to, random_int(1, 999999), $msgBody, $message->getFromAccount());
            }
            $request->setSyncOtherMachine(SyncOtherMachine::OUT_OF_SYNC);

            // 设置离线推送消息内容
            $offlinePushInfo = $this->getOfflinePushInfo($message, TencentCloudChatType::PRIVATE_CHAT);
    ...
    public function getOfflinePushInfo(TencentCloudImMessage $message): OfflinePushInfo
    {
        ...
        if (config('extension.tencent_cloud.tim.offline_push.android_info.oppo_channel_id')
            ...
            $androidInfo->setXiaoMiChannelId(config('extension.tencent_cloud.tim.offline_push.android_info.xiaomi_channel_id'));
